...
INFO_OUTPUT    = true   -- Activate/Deactivate console info  prints
ALWAYS_RUNNING = true   -- Triggers always running despite as long as client is connected
CMD_UPDATE_MS  = 50     -- Update rate to check for incoming commands

--TD
FINGER_LEFT        = 1        -- Index for left finger configuration
FINGER_RIGHT       = 0        -- Index for right finger configuration
TD_INIT_TIME       = 3000     -- Time wait for TD initialization - 1/2 after off, 1/2 after init per finger
...
DEFAULT_COND_REL_ACTIVE      = true;    -- Default activation state of conditional checks before releasing
DEFAULT_TD_LEVEL             = 50;      -- Default TD threshold level to seen as active node
                    ...
                    while ((grip_ok ~= true) and (next_force <= DEFAULT_MAX_FORCE)) do
                        ...
                        while ((grip_ok ~= true) and (data_try_count <= DEFAULT_TD_IDENTIFICATION_LIMIT)) do
                            ...
                            if ((data_l[1] ~= -1) and (data_r[1] ~= -1)) then
                                ...
                                if data_try_count == DEFAULT_TD_IDENTIFICATION_LIMIT then
                                    retry = true
                                end
                            end
